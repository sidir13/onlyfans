...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if mqtt_enabled:
        try:
            ...
            # CORRECTION : Utilisation des bonnes variables d'environnement Docker Compose
            host = os.environ.get("MQTT_BROKER_HOST", "mosquitto")
            port = int(os.environ.get("MQTT_BROKER_PORT", "1883"))
            # CORRECTION : Passage des paramètres sous forme de dictionnaire d'options
            publisher = MqttPublisher({"broker_host": host, "broker_port": port})
