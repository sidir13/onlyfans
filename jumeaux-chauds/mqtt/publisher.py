...
class MqttPublisher:
    ...
    async def publish_telemetry(self, snapshot: dict) -> None:
        ...
        machine_id = snapshot["id"] #machine_id = snapshot["machine_id"]
        payload = {
            "schema_version": "1.0",
            "ts": _now_iso(),
            **snapshot,
        ...
        for sensor_data in snapshot.get("sensors", []):
            await self._publish(
                self._t(cluster_id, machine_id, "temp", sensor_data["sensor_id"]),
                {"ts": payload["ts"], "value_c": sensor_data["temp_c"]},
                qos=self._qos_telemetry,
            )
